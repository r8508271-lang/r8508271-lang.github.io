...
`data/policy-examples.json` contains 76 verified clips across 28 environments: 21 environments have three available methods, six have two, and BaseMotion has one. Each environment uses the same held-out instance across methods. Captions show replay outcomes and action counts; selected clips are not representative averages. Backgrounds are enabled in dynamic 3D scenes. Clip provenance records program, evaluation, initial-state/frame, and media checksums. Missing clips are explicitly identified.
